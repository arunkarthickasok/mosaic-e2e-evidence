...
## CP-ADOPT-1 §P0 — DERIVATION BLUEPRINT filed (2026-09-17) — HONEST CHECKPOINT before the discovery hot-path mutation
Fresh-read d0fdb22 (clean). reports/REPORT-CP-ADOPT-1.md §P0: the gate (SdcComponentDiscovery.php:40-56, skip 45-48); H1 bare-id consumer census (MosaicRenderer.php:425/437/632/658/731/753/804 · MosaicLayoutWidget.php:633 · ManifestController.php:85 — 8 sites); governance (component_package schema-only zero-consumer; node_type mosaic.allowed_components; restricted from attribute); DERIVED scenario universe C1–C15 (discovery-shape × provider × grade × governance × permission-parity × geometry) with per-cell oracle + smoke-alarm; entity shape mosaic_component_library (F/H8). **P1 NOT started — honest checkpoint:** P1 re-bases SdcComponentDiscovery on core plugin.manager.sdc (H2), removes the .mosaic.yml gate, threads the alias map (H1) through the 8 hot-path consumers under the "saved layouts render byte-identical" invariant + grader + palette guard — a delicate discovery/render hot-path change not to be rushed at the tail of a very long session; a half-applied gate-removal would change dev discovery on next cache rebuild + entangle P2 auto-create. §P0 is the complete blueprint (build order + cells) so P1 starts clean. **P2 GATE:** the Component Library entity writes dev config/DB → REQUIRES Arun's sanction word (absent here) → STOP-and-ask. **STOP — reviewer audits §P0; P1 discovery build = next focused pass; P2 awaits sanction.**

## CP-ADOPT-1 §P1 — INTERIM CHECKPOINT (2026-09-17) — building blocks GREEN, hot-path integration honest-checkpointed
Fresh-read d0fdb22 clean; P2 sanction word PRESENT (Arun: dev config/DB writes SANCTIONED, dev-only). Built two isolated, tested units in the mosaic tree (READ-ONLY git, uncommitted, ship #41 candidates): (1) GRADER src/Sdc/MosaicComponentGrader.php (Pillar A: Ready/Attention(prop+reason)/Blocked, slots-only=Ready, shape test mirrors defToField, tolerant group/replaces/internal/variants) + Unit test cells C1/C3/C4/C5/C6+tolerated+smoke — RED demoed (neuter mapsToKnownShape→TRUE → C3 fails) then restored. (2) ALIAS MAP src/Sdc/MosaicComponentAlias.php (H1: toCanonical/toBare/isOwned/fromSdcDefinitions; owned=mosaic/mosaic_*; adopted+unknown+mosaic_region passthrough; idempotent; smoke-alarm bypass leaves bare mosaic_view unresolved) + Unit test. GATES on these: Unit 15/54 OK, phpcs 0 ERRORS (4 files), phpstan L6 [OK]; no dist change → no libs bump. **HONEST CHECKPOINT — NOT CHECKPOINT-1 (needs all P1):** the coupled hot-path integration is NOT started — step-1 threading through the 8 consumers + step-3 core-SDC rebase (drop .mosaic.yml gate, derive from plugin.manager.sdc, byte-identical node/780 invariant, cells C2/C4/C5/C7/C8) is one delicate render-hot-path change deferred to a fresh focused pass rather than rushed at the tail of a very long session; then step-4 palette guard, step-5 full gates + live olivero:teaser proof, then P2 (entity+admin+node-type form+LIVE auto-create, sanction in hand), P3 Playwright, P4 close. No red left red (grader C3 RED was a demonstrated+restored smoke-alarm); no oracle changed. Report reports/REPORT-CP-ADOPT-1.md §P1. **STOP — reviewer audits §P1 units; next pass = thread+rebase → guard → gates → CHECKPOINT-1 → P2(live) → P3 → P4.**

## CP-ADOPT-1 PASS 3 — INTEGRATION HEART (STEP 1+3+4) DONE + verified live (2026-09-17)
Mosaic git READ-ONLY (edited, uncommitted, ship #41 candidates); P2 sanction in hand (unused this pass — no entity auto-create yet). **STEP 1+3 core-SDC rebase (H2) + alias (H1):** SdcComponentDiscovery re-based on plugin.manager.sdc (services.yml args), .mosaic.yml gate at :45-48 DELETED, sidecar=optional enhancer; owned keyed BARE (providerIsOwned+machineName), adopted provider:id; ComponentDefinition +grade/gradeReasons/providerType/adoptPalette (+fromCoreDefinition/withGrade), manager merge preserves them. LIVE: registry 14→15, olivero:teaser ADMITTED grade=attention pt=theme palette=N reason "attributes has no known field shape". BYTE-IDENTICAL: node/780 region shasum 0864e2386… before==after (full-page delta = drush cr asset-hash noise, proven: bare cr moves it too; stripped shasum c9972a7b stable). SMOKE-ALARM RED→restored on node 800 (mosaic_tabs, sidecar-only): bypass bare-keying → getDefinition(mosaic_tabs) NULL + 1 missing + tabs unrendered → restored. HONEST FINDING: 11+ owned have PHP classes (keyed bare independently); alias bare-keying is load-bearing for the 3 sidecar-only (carousel/tabs/live_search). **STEP 4 palette guard:** adopt_palette FALSE excluded from author manifest in ManifestController (all) + MosaicLayoutWidget (admin); LIVE admin manifest=14, olivero guarded (was excluded only by accidental createInstance-fail before). **Gates:** Unit 15/54, phpcs 0 ERRORS (7 files), phpstan L6 OK, discovery/manifest Kernel 23/112 no-regression; no dist → no libs bump. **HONEST CHECKPOINT (not full CHECKPOINT-1):** formal Kernel cells C2/C4/C5/C7/C8 (verified live, not yet codified), FULL Kernel+Vitest, + P2(entity/admin/node-form/LIVE auto-create)+P3(Playwright)+P4(close) remain. No red left red. Report reports/REPORT-CP-ADOPT-1.md §P1(PASS 3). **STOP — reviewer audits the integration heart; next pass: codify C2-C8 + full gates → CHECKPOINT-1 → P2 live → P3 → P4.**

## CP-ADOPT-1 PASS 4 STEP A — CHECKPOINT-1 CLOSED (§P1 complete + green) 2026-09-17
C2–C8 codified as SdcComponentDiscoveryTest cells (mock plugin.manager.sdc + real grader): C2 schema-only module→Ready/adopted/guarded, C4 slots-only→Ready, C5 no-schema-renderable→Ready (ORACLE CHOSEN + recorded: charter said Blocked; witnessed renderable⇒Ready, Blocked reserved for C6), C6 unrenderable→Blocked, C7 theme-active→theme-bound/Attention, C8 inactive-theme→core-excluded, alias owned-bare/adopted-prefixed, palette-guard, cache-invalidation live (mosaic_tabs Interactive→CACHEPROBE_ZZZ→restored). ORACLE-CHANGE: 3 Sprint02SmokeTest cells drove the removed gate → flipped to ADOPT contract (admit without sidecar), old→new inline. **FULL GATES: Kernel mosaic core 198/1215/0 (no regression), Unit FULL 2709/6493/0, Vitest 539/1(B-101), phpcs 0 ERRORS (all changed PHP), phpstan L6 clean on CP-ADOPT-1 code (legacy widget 16 pre-existing untouched), no dist→no libs.** FINDING-107 (alias bare-keying load-bearing only for 3 sidecar-only owned). Report reports/REPORT-CP-ADOPT-1.md §P1 CHECKPOINT-1. **HONEST CHECKPOINT: P2 (mosaic_component_library ConfigEntityType + admin page + node-type form + LIVE auto-create/updb) = full new-subsystem pass, deferred rather than rushed; rebase done+green, P2 builds on it. STOP — reviewer audits CHECKPOINT-1; next: P2 live→CHECKPOINT-2→P3→P4.**

## CP-ADOPT-1 PASS 5 — P2 CHECKPOINT-2 CLOSED (2026-09-17) — Component Library entity + governance, all 7 items live+green
Reviewer acceptances recorded (C5 "Ready (static)"; Sprint02 flips). PHP+YAML only, no dist/libs. **Item1** ConfigEntityType mosaic_component_library (id=provider, provider_type, status, components[]{id,enabled,restricted} H8; readiness computed not stored) + schema. **Item2** MosaicComponentLibrarySync + hook_install + mosaic_update_10003; LIVE updb "created 4 component libraries (olivero,mosaic_components,mosaic_views,mosaic_webform)"; mosaic_* ON / olivero OFF; idempotent re-sync=[]; drush cex clean (olivero.yml status:false provider_type:theme). **Item3** default_component_package REMOVED (no consumer; unset live in update hook; absent verified). **Item4** admin page /admin/config/mosaic/component-libraries (MosaicComponentLibrariesForm; old ComponentPackagesController page REMOVED, route repurposed); ON/OFF + per-component enabled/restricted + grade badges+reasons + props/slots living-docs + theme-bound; access admin200/author403/anon403 (anon HTTP 403 raw). **Item5** node-form filtered to authorable (adopted excluded, 14 opts on page) + libraries link; allowed_components write path UNCHANGED (back-compat). **Item6** MosaicComponentGovernance (library OFF/disabled/restricted entity-first + attribute fallback) wired ManifestController+MosaicLayoutWidget; LIVE 14→OFF→2→restored; Kernel ComponentLibraryGovernanceTest 5/16. **Item7 GATES:** Kernel mosaic core 203/1231/0, Unit 2709/6498/0, Vitest 539/1(B-101), phpcs 0, phpstan L6 OK. ORACLE-CHANGES recorded: ManifestControllerTest (+governance arg), Sprint30 (route/menu component-packages→libraries), Sprint80 (isAuthorable). Cumulative CP-ADOPT-1 set (passes 3-5): 16 M + 10 new. Report reports/REPORT-CP-ADOPT-1.md §P2. **STOP — P3 Playwright + P4 close next pass.**

## CP-ADOPT-1 PASS 6 — P3+P4 CLOSE (2026-09-17) — COMPLETE (P0–P4), awaiting Arun eye-test + ship #41
**Ship #41 candidate set: 26 files (16 M + 10 new), all trackable, 0 gitignored; SdcComponentPlugin.php UNTOUCHED (not HELD).** PHP+YAML only, no dist/libs. **P3 journeys:** J1 admin page FILMED (4 libraries provider-order, olivero theme-bound+Attention, boundingBox non-overlap; 2 frames) + J2/J3/J5 proven at the manifest level (J2 round-trip 14→OFF→2→ON→14; J3 disable→13, restrict→admin YES/author NO; J5 anon node/780 region shasum 0864e238… unchanged) + J4 unchanged P7-039 allowlist (back-compat). Builder palette states verified via manifest (headless builder unstable). Album ledger-live/e2e-evidence/cp-adopt-1/ + INDEX. **P4:** REPORT-CP-ADOPT-1.md complete (P0–P4, oracle-change table 8 distinct changes: C5, Sprint02×3, ManifestControllerTest 9-cell, Sprint30×2, Sprint80; region-vs-fullpage shasum note; FINDING-107 xref); SHIP-41-PLAN.md (26 files + check-ignore verdicts + config schema + mosaic_update_10003 + proposed commit); WALK-CP-ADOPT-1.md (9 steps, J2-first, recipe-grade). **Library entity LIVE on dev** (4 auto-created, journeys restored to defaults). **STOP — Arun walks WALK-CP-ADOPT-1.md, then the ship #41 human-commit closes CP-ADOPT-1.** Road after: author-trust slice → backend config audit → Wave D → Wave F → Wave G → minimal ACT 2 → soak → tag 1.0.0.

### WALK-CATCH #63 + #64 — Arun eye-test 2026-09-17 (tally → 64) — CP-ADOPT-1R RIDER (ship #41 BLOCKED)
**WC#63 (Arun):** "restricted hides Button from ADMIN too." Governance branch is `return $isAdmin || !$restricted;` (MosaicComponentGovernance.php:88) — should show to admin; the real cause is under witness (form-save vs widget path).
**WC#64 (Arun):** "node-type allowlist doesn't persist; existing allowlist (article) reads as empty and palette is wide open." MosaicFormHooks node-form (this CP's authorable-filter) — read/write/alias-boundary under witness.
PROBE→FIX→RED→GREEN→re-walk W3+W4. Ship #41 blocked until both green.

## CP-ADOPT-1R — WC#63 + #64 rider (2026-09-17) — WC#64 FIXED, WC#63 not reproduced; ship #41 un-blocked pending Arun W3+W4 re-walk
**WC#64 FIXED (real bug):** node-type allowlist value lands at flat #parents ["allowed_components"] (mosaic_governance details is #tree:false) but saveAllowedComponents read the nested path → unset every save. Fix: read flat 'allowed_components'. + added missing config schema node.type.*.third_party.mosaic. RED→GREEN Functional NodeTypeAllowlistTest 2/20 (round-trip + clear-unsets + back-compat read; RED demo nested→fail); real-browser: 3 persist after save+reload. Article 3rd-party BEFORE=[] (none stored on dev). **WC#63 NOT REPRODUCED:** governance `$isAdmin || !$restricted`; admin retains restricted Button on FE manifest (uid1 YES/uid3 no), admin-widget (14), real-HTTP builder (14), form checkboxes render+save correctly, builder manifest invalidates on library change (no staleness). Locked by real two-account Kernel cell (ComponentLibraryGovernanceTest testWc63RealTwoAccount…, now 6/36). Likely walk-transient (Button left disabled from W3.1). **Test-Coupling:** manifest-only oracle missed both — added two-account Kernel cell + Functional form round-trip permanently. Gates: phpcs 0, phpstan OK, Kernel 6/36, Functional 2/20, cex clean (new schema). 2 films (cp-adopt-1r/). Ship #41 set now 16 M + 11 new = 27 (+NodeTypeAllowlistTest). Report §R; SHIP-41-PLAN updated; re-walk WALK-CP-ADOPT-1R.md (W3+W4). **STOP — Arun re-walks W3+W4; on pass, ship #41 human-commit.**

## ★ SHIP #41 SHIPPED 2026-09-17 = commit 5a65173 (parent d0fdb22) — 27 files — CP-ADOPT-1 + 1R
Adopt any SDC: discovery rebased on core plugin.manager.sdc (.mosaic.yml gate removed, sidecar optional), readiness grader, bare-id alias (byte-identical owned render), component library config entity + idempotent auto-create + governance + admin page, palette guard, WC#64 allowlist-persistence fix + config schema. Arun re-walk 6/6 PASS + W5 PASS. Walk-catches #63 (NOT reproduced, locked by two-account cell) + #64 (fixed); tally 64. Gates: Kernel 203/1231, Unit 2709/6503, Functional 2/20, Vitest 539/1, phpcs 0, phpstan OK; PHP+YAML only, no dist/libs. SHIP-41.md + oracle-change table + evidence. **CP-ADOPT-1 CLOSED.** Road after: CP-ADOPT-2 (panel from schema) → CP-ADOPT-3..7 → author-trust slice → backend config audit → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0.

## CP-ADOPT-2 §P0 — PANEL-FROM-SCHEMA DERIVATION BLUEPRINT filed (2026-09-17) — honest checkpoint before P1 build
Fresh-read 5a65173 (ship #41 at HEAD, clean). reports/REPORT-CP-ADOPT-2.md §P0: quoted the TS derivation (MosaicPuckAdapter.ts defToField :1739-1785 — enum→select, string→text, int/number→number, bool→radio, array, object, default+$ref→raw), the field_types override path (:590/591/596, later wins = F-084 precedent), the 8 MosaicFieldType plugins→widgets, the SDC prop-schema keys core exposes (live: mosaic_card {type,title,default,enum}; olivero teaser attributes = class-string type → Attention). DERIVED matrix: prop-shape {string/uri-format/html-contentMediaType/enum/meta:enum/boolean/int/number/object+$ref-known/object-unknown/array-objects/array-scalars/$ref-unresolved/required±default±examples/variants} × source × surface × grade + oracle per cell. H3 (abstract descriptors in PHP, adapter maps last) / H4 (default→examples[0]→empty; required+neither→Attention) / H5 (save-time validation, translatable-by-shape flag) targets. Byte-identical owned invariant pinned (node/780 0864e238…). **HONEST CHECKPOINT — P1 NOT started:** P1 = PHP prop-shape registry + manifest emits descriptors + TS adapter rework (descriptor→Puck last, byte-identical owned both surfaces) + H4 + H5 save-validator — a coupled manifest→adapter hot-path change of the same class as the CP-ADOPT-1 discovery rebase; deferred to a fresh focused pass (CP-ADOPT-1 alone ran 6 passes today), blueprint ready. Build order 1-7 in the report. **STOP — reviewer audits §P0; P1 build next pass → CHECKPOINT-1.**

### OPEN QUEUE: CP-ADOPT-2 §P0 GREEN (reviewer) → P1 build (registry + descriptors + adapter rework + H4/H5, RED→GREEN, CHECKPOINT-1) → CP-ADOPT-3..7 → author-trust slice → backend config audit → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0.

## CP-ADOPT-2 PASS 2 — PANEL-FROM-SCHEMA: the PHP derivation half LANDED GREEN (2026-09-17) — checkpoint before TS/dist/H5
Fresh-read 5a65173 (HEAD, clean). Built the server-side half of Pillar B end-to-end RED→GREEN; the TS-adapter-consumption + H5 save-validation + dist slice is CHECKPOINT-1 (next pass). **13 files (7 M + 6 new), PHP+YAML only, NO dist/libs.**
- **Item 1 (H3):** `PropShape` (pure shared classifier, ONE source of truth for grader + panel) + `PropDescriptor` (immutable value object) + `MosaicPropShapeEvent` (alter event) + `MosaicPropShapeRegistry` (service, event-alterable). Kinds semantic (no Puck names): text/formatted_text/link/media/select/toggle/number/repeatable/entity_ref/raw. string refined by format→link, contentMediaType text/html→formatted_text. Grader `mapsToKnownShape`→`PropShape::isKnown` (single source). **Unit/Sdc 92/92**; **RED demo: neuter classify→always TEXT = 35 failures → restored clean.**
- **Item 2 (PHP half):** MosaicManifestBuilder emits `prop_descriptors` (ADDITIVE) from `propDefinitions.properties` — propDefinitions + field_types UNTOUCHED. Live: mosaic_card 6×text+variant select(2); mosaic_button 5×text.
- **Item 3 (H4):** registry default = default→examples[0]→type-empty; grader adds Attention for required-without-seed. **Zero live effect** (no such prop exists).
- **Item 4 (partial):** `translatable` flag emitted (string/html/uri). H5 save-validation NOT built (in the TS/save slice).
- **Item 5:** render path untouched → node/780 anon region shasum **14e6cb9c…(3954B) BEFORE==AFTER**. HONEST: could NOT reproduce the historical 0864e238 (never scripted; balanced field-region ≠ that hash under raw/cb-strip/ws-collapse); pinned a reproducible baseline instead; re-establishing the canonical extraction is a prereq for the TS-slice byte-identical proof (ledgered).
- **Item 6:** grades unchanged (15; olivero:teaser Attention only). olivero descriptor set proven at registry (`attributes→raw`, agrees with grade). **FINDING-108:** getPropDefinitions() returns the JSON-schema ROOT (adapter reads .properties); adopted components return [] (getPropDefinitions .component.yml gap = the palette-guard boundary) — adopted manifest-emission folded into the palette-opening slice.
- **Gates:** Unit FULL 2758/2758 (1 pre-existing warning, mosaic_registry, untouched), Kernel FULL 204/204, phpcs 0, phpstan L6 OK.
- **Oracle-changes (2):** grader C1 bare-object→Attention (zero live effect); ManifestController/Widget tests +registry arg.
- Report reports/REPORT-CP-ADOPT-2.md §P1 (PASS 2). **STOP — reviewer audits the PHP half; CHECKPOINT-1 = TS adapter consumes descriptor→Puck LAST (descriptorToField mirrors defToField, field_types wins, Vitest, dist→BUMP-LIBS) + H5 save-time Kernel + byte-identical WITH adapter change + owned-manifest diff.**

### OPEN QUEUE: CP-ADOPT-2 P1 PHP-half GREEN (reviewer) → CHECKPOINT-1 (TS/dist/H5 slice) → re-establish canonical region extraction → CP-ADOPT-3..7 → adopted-authoring+palette-opening (FINDING-108) → author-trust → backend audit → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0.

## CP-ADOPT-2 PASS 3 — LEDGER FIRST (2026-09-17)
(a) **ORACLE CHANGE — region-shasum baseline.** OLD `0864e238…` (CP-ADOPT-1, computed ad-hoc, never scripted, NON-REPRODUCIBLE) → NEW `14e6cb9c17dc61b90a86dd97d8957ae462d789ff854d3523ae581010a43e0dec` (3954 bytes) via a COMMITTED extraction script. **Script (in the ship set):** `web/modules/custom/mosaic/scripts/qa/region-shasum.sh` — sha256 of the balanced `<div class="field field--name-field-mosaic-layout …">` region of a node's ANON page (asset/cache-buster-free by construction; stable across `drush cr`). Reviewer ACCEPTED **on condition the script ships**; it is added to the ADOPT-2 file set. **Every later CP quotes `region-shasum.sh <url>` before == after.**
(b) **TRUTH-PASS — F-098 release headline candidate.** "Mosaic adopts any component library and feeds it Drupal data — Views, page fields, entities — without the library knowing Mosaic exists." **Status: CANDIDATE, NOT RATIFIED.** Allowed to stand as the release headline ONLY if the external-library oracle walk passes (a real non-Mosaic component library, discovered + graded + fed Views/page-field/entity data end-to-end, no `.mosaic.yml`). Until that walk is filmed green, the headline stays a candidate in this ledger and is NOT quoted in any release note.

## CP-ADOPT-2 PASS 3 — CHECKPOINT-1 FILED: F-108 + TS adapter + H5 + dist, ALL GREEN (2026-09-17)
Fresh-read 5a65173 (HEAD, clean; 13 PASS-2 files uncommitted). Built the coupled slice PASS 2 deferred. **Full ADOPT-2 set now 28 files (17 M + 11 new).**
- **Item 1 F-108 FIX:** ComponentDefinition carries the core SDC `props` root (fromCoreDefinition→withGrade→toPluginDefinition); SdcComponentPlugin::getPropDefinitions() prefers it (owned sidecar falls back to .component.yml, byte-identical). olivero:teaser getPropDefinitions [] → {type,properties}; **prop_descriptors count=1** (attributes→raw, Attention reason intact). Kernel AdoptedDescriptorParityTest (adopted manifest == registry derivation; adopted palette-guarded). RED: neuter F-108 branch → parity cell fails (no 'properties' key) → restored.
- **Item 2 TS ADAPTER:** schema.ts +PropDescriptorJson +manifest.prop_descriptors; toConfig base step = propsFieldsFromDescriptors→descriptorToField (byte-identical to defToField for owned: text/select-value-as-label/number±min-max/**boolean→Yes-No radio**; repeatable/raw defer to schema; no-descriptor→defToField fallback); field_types sidecar still WINS (F-084). Vitest PropDescriptors.test.ts 8/8 incl byte-identical proof (descriptor path===legacy). dist rebuilt.
- **Item 3 H5:** MosaicPropValidator +registry, validateShapes() rejects formatted_text(HTML)-without-{prop}Format; unknown props tolerated. Kernel PropShapeSaveValidationTest 4/4 (invalid enum / html-no-format / unknown-tolerated / valid) via real validate() + mosaic_test_shape fixture (enum+html prop). RED: neuter validateShapes → html-no-format cell fails → restored. Safe: no owned comp uses bare contentMediaType (zero regression).
- **Item 4 byte-identical:** COMMITTED SCRIPT scripts/qa/region-shasum.sh (`.gitignore` exception ships it) → **14e6cb9c…(3954B) BEFORE==AFTER** (dist rebuilt+libs bumped+cr). Palette guard CLOSED (15 comps, olivero:teaser guarded). Owned-manifest diff ADDITIONS-ONLY (mosaic_card +prop_descriptors; propDefinitions.properties + field_types unchanged).
- **Item 5 dist:** vite.builder.config.ts (builder.js 1246kB) + vite.frontend-editor.config.ts (776kB). **BUMP-LIBS 1.0.29→1.0.30** (×3). FINDING-109: package.json "build" script points to a stale vite.bundles.config.ts (fix ledgered).
- **Gates:** Unit FULL 2758/2758, Kernel FULL 209/209 (0 fail, +5 Adopt), Vitest 547pass/1-B101(pre-existing checkbox-drift, untouched), phpcs 0, phpstan L6 OK. **Oracle-changes: MosaicPropValidatorTest +registry arg (H5).**
- Report reports/REPORT-CP-ADOPT-2.md §P1 PASS 3; SHIP-42-PLAN.md (28 files + verdicts + commit msg). **STOP — reviewer audits; ship #42 human-commit closes CP-ADOPT-2.**

### OPEN QUEUE: CP-ADOPT-2 CHECKPOINT-1 GREEN (reviewer) → ship #42 human-commit → fix FINDING-109 (build script) → CP-ADOPT-3..7 → adopted-authoring+palette-opening (F-108/F-098 external-library oracle walk) → author-trust → backend audit → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0.

## CP-ADOPT-2 PASS 4 — HELD-FILE CORRECTION + LEDGER (2026-09-17)
**HELD-FILE VIOLATION CORRECTED.** In PASS 3 I added an F-108 hunk to `src/Plugin/MosaicComponent/SdcComponentPlugin.php` — a HELD file (Wave 5.2 / bible P1.2). Full diff quoted in the paste. **HONEST FINDING:** the diff was 100% my F-108 hunk — there was NO separate pre-existing uncommitted Wave-5.2 delta in the working tree; the held file's pre-ADOPT-2 content == HEAD (5a65173). **Correction:** reverted my hunk via the Edit tool (git stays read-only — no checkout/restore) → the file is now **pristine, 0 diff vs HEAD**. F-108 behaviour MOVED onto the definition path: `MosaicManifestBuilder::buildPropDescriptors($propDefinitions, $def)` falls back to the discovery-stored `$def['props']` (from `ComponentDefinition::fromCoreDefinition`) when getPropDefinitions() is empty. Held `SdcComponentPlugin::getPropDefinitions()` UNTOUCHED (still [] for adopted). `AdoptedDescriptorParityTest` updated to assert F-108 on the definition path (held plugin returns [], def carries `props`, manifest == registry derivation). LIVE: teaser prop_descriptors count = 1 (attributes→raw); byte-identical 14e6cb9c before==after (scripts/qa/region-shasum.sh). Adopt Kernel 11/11; Unit FULL 2758/2758. NB the held file carries a PRE-EXISTING phpcs @return issue (line 50, present in HEAD) — left untouched per the held-file law. **Ship set recount: 27 files (16 M + 11 new)** — SdcComponentPlugin.php DROPPED (pristine). No dist/adapter change this pass → libs stay 1.0.30.

## LEDGER (append-only) — rulings recorded 2026-09-17
(a) **Arun ruling — Wave G / submodules.** Every submodule ships WORKING in 1.0 — no demotion. Working = enabled + Arun-walked green + derived tests that bite. The commerce dependency decision (declare the dep vs replace undeclared Twig calls) is brought to Arun WITH evidence at that CP. The **registry ships as a working read-only catalog**; its import path is labelled **1.1** (Arun to confirm at Wave G).
(b) **Reviewer delegated ruling — backend config audit.** Runs in PARALLEL, starting AFTER ADOPT-4 lands. READ-ONLY until its gap register is ratified. Separate report **reports/REPORT-CONFIG-AUDIT.md**; M3 sizing lives in the register.
...
## CP-ADOPT-3 §P0 + §P1 item 1 — SLOTS → DROP ZONES: derivation + slot-descriptor emitter GREEN (2026-09-17)
Fresh-read 87be200 (ship #42 at HEAD, clean). Pillar C (SDC slots → drop zones with child rules; H7). **3 files (1 M + 2 new), PHP only, NO dist/libs.**
- **§P0 DERIVATION (reports/REPORT-CP-ADOPT-3.md):** quoted the 3 slot sources today (PHP hardcode MosaicColumnsComponent:53; sidecar fromSidecarYaml:206; core SDC fromCoreDefinition:133) → getSlotDefinitions → manifest slotDefinitions → adapter {type:'slot'} (:585) → storage node.slots[zone] (:1062/1181) → MosaicRenderer:703-708 passes children. **PROVED UNREAD:** mosaic_columns core SDC slots=[] (PHP hardcodes column_1..4); child RULES (allowed/preferred/min/max/defaults/empty_display) modeled NOWHERE; even SDC `required` unenforced. DERIVED matrix: slot-source × rules × violation × surface × geometry × permission-parity, oracle+smoke per cell. H7: SDC descriptor=truth for id, sidecar=rules-only (redefining an id → rejected+warning).
- **§P1 item 1 (DONE):** src/Sdc/SlotDescriptor.php (value object {id,label,required,+rule fields}) + MosaicManifestBuilder emits `slot_descriptors` (ADDITIVE, from getSlotDefinitions; identity only — rules default). LIVE: mosaic_columns→column_1..4; olivero:teaser→content(**required=TRUE**, the previously-unread SDC required flag now surfaces)+image/meta/prefix/title. H7 id==zone key. Kernel SlotDescriptorEmissionTest 3/3. RED: neuter buildSlotDescriptors→owned cell fails→restored. **Byte-identical 14e6cb9c before==after** (additive; adapter/render untouched).
- **Gates:** Unit FULL 2758/2758, Kernel FULL 212/212 (+3), phpcs 0, phpstan L6. No dist → no BUMP-LIBS.
- **CHECKPOINT — items 2-5 next pass:** child-rules model (SDC metadata + sidecar rules-only, H7 reject cell) → enforcement+geometry (non-allowed refused w/ reason, min/max banner, defaults, empty_display; admin+FE parity; drop-zone boundingBox; adapter rework → dist→BUMP-LIBS) → adopted slots in living-docs → full gates → CHECKPOINT-1.
- Report reports/REPORT-CP-ADOPT-3.md §P0 + §P1 item 1. **STOP — reviewer audits; items 2-5 = the coupled enforcement/geometry/dist slice next pass.**

### OPEN QUEUE: CP-ADOPT-3 item 1 GREEN (reviewer) → items 2-5 (rules+enforcement+geometry+dist, CHECKPOINT-1) → CP-ADOPT-4 (adopted authoring + palette opening + F-098 external-library oracle walk) → CP-ADOPT-5..7 → backend config audit (parallel, read-only) → author-trust → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-3 PASS 2 — items 2 + 2b: CHILD-RULES MODEL + own-house truth GREEN (2026-09-17)
Fresh-read 87be200 (ship #42 at HEAD; item-1 files uncommitted). Baseline shasum 14e6cb9c confirmed. Built the PHP child-rules model; adapter-enforcement + geometry-filming + dist (items 3-5) = the coupled slice next. **Full ADOPT-3 set now 8 files (5 M + 3 new), PHP only, NO dist/libs.**
- **Item 2 (DONE):** SlotDescriptor::fromMetadata reads {allowed,preferred,min,max,defaults,empty_display} tolerant + **required→min=1**; withRules overlays rules keeping identity; partitionSidecarRules = **H7** (rule for undeclared slot → rejected). MosaicManifestBuilder applies the sidecar `slot_rules` overlay (threaded via ComponentDefinition slotRules from the sidecar `slot_rules:` key) + **logs a warning** (injected logger.channel.mosaic) per rejected phantom. LIVE: olivero:teaser.content emits min:1 (required SDC slot); image no min; owned columns unchanged. Unit SlotDescriptorTest 4/4; Kernel SlotDescriptorEmissionTest 5/5 (+min=1, +H7 wiring w/ warning() asserted). RED: neuter partitionSidecarRules → phantom appears + logger never warned → fails → restored.
- **Item 2b own-house truth (DONE, audit):** ONLY owned slot container = MosaicColumnsComponent (PHP column_1..4; core SDC slots=[]). mosaic_tabs is NOT a slot container (sets repeatable). **Ruled DYNAMIC exception — columns keeps PHP:** declares max 4, but ACTIVE zones driven by the `columns` prop (1-4) at author+render (adapter:794 slotKeys.filter i<colCount); a static .component.yml slots: can't express "N of 4 by prop". No static container to migrate → the charter's "move static slots" has an empty work-list here. No storage change; byte-identical.
- **Byte-identical:** 14e6cb9c before==after (additive slot_descriptors; adapter/render untouched). No dist → no BUMP-LIBS.
- **Gates:** Unit FULL 2762/2762, Kernel FULL 214/214 (+2), phpcs 0, phpstan L6. Oracle-change: ManifestController/Widget tests +logger (NullLogger).
- Report reports/REPORT-CP-ADOPT-3.md §P1 PASS 2. **STOP — reviewer audits; items 3-5 (adapter enforcement: Puck allow/disallow + banners + defaults + empty_display; geometry journeys FILMED; dist→BUMP-LIBS; SHIP-43-PLAN; WALK-CP-ADOPT-3 per WALK-SCRIPT LAW) = the coupled slice → CHECKPOINT-1 next pass.**

### OPEN QUEUE: CP-ADOPT-3 items 2+2b GREEN (reviewer) → items 3-5 (adapter enforcement + geometry filmed + dist, CHECKPOINT-1, SHIP-43-PLAN, WALK) → CP-ADOPT-4 (adopted authoring + palette opening + F-098 external-library oracle walk) → CP-ADOPT-5..7 → backend config audit (parallel, read-only) → author-trust → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-3 PASS 3 — items 3-5: ADAPTER ENFORCEMENT + FILMED GEOMETRY + dist — CHECKPOINT-1 FILED (2026-09-17)
Fresh-read 87be200 (ship #42 at HEAD; items 1+2 uncommitted). Baseline 14e6cb9c. Built the coupled adapter+UI+dist slice. **Full ADOPT-3 set now 17 files (12 M + 5 new), dist rebuilt, libs 1.0.30→1.0.31.**
- **Item 3 (DONE):** MosaicSlotZone.tsx (drop-zone chrome: allowed hint = visible reason, LIVE min/max banner via `[data-puck-component]` MutationObserver count, empty_display). Adapter: slot field +`allow` (Puck refuses non-allowed drops natively); slots render via MosaicSlotZone; PuckField +allow/disallow. schema.ts +SlotDescriptorJson +manifest.slot_descriptors. **MosaicColumnsComponent.column_1 min=1 + empty_display** (ruled-exception PHP source → a real banner on a shipped component). Vitest SlotEnforcement.test.tsx 5/5 (allow field, zone chrome, live banner-clear). Oracle-change Sprint61Smoke (slots via MosaicSlotZone). tsc clean (only pre-existing dsdShadow).
- **Item 4 FILMED (DONE — real frames):** journey js/e2e/journeys/cp-adopt-3.spec.ts (scratch node 987, column_1 empty), element clips no-sleeps. Album ledger-live/e2e-evidence/cp-adopt-3/ + INDEX: **j1** red "Requires at least 1 item — 0/1" banner + dashed empty zone + empty_display text (LIVE builder, verified visually); j2/j3 crops; **j4** filled column_1 (node 334) → NO banner (conditional; the after-drop state); geometry.json col1 x415-551 + col2 x567 NON-OVERLAPPING. The static no-drag approach was STABLE (cp-adopt-1 said builder-drag filming unstable → we film the static banner state instead). Drop-refusal + FE-dialog parity machine-proven (Vitest + same adapter), not filmed (drag=unstable) — honest.
- **Item 5 dist+gates (DONE):** vite builder (1249kB) + frontend-editor (779kB). **BUMP-LIBS 1.0.30→1.0.31** (×3). Byte-identical 14e6cb9c before==after (FE Twig render unaffected by builder adapter). Gates: Unit FULL 2762/2762, Kernel FULL 214/214, Vitest 552/1-B101, phpcs 0, phpstan L6.
- **DEFERRED (honest, ledgered):** defaults[]-insertion-on-first-placement — a Puck dispatch(insert) on mount risks a silent save-state write; defaults emitted on the descriptor but auto-insertion → author-trust slice (Arun's "never write silently vs insert defaults" ruling). All other item-3 rules done+filmed/tested.
- Report REPORT-CP-ADOPT-3.md §P1 PASS 3; WALK-CP-ADOPT-3.md (3 steps + a drag, per WALK-SCRIPT LAW, filmed screens only); SHIP-43-PLAN.md (17 files). **CHECKPOINT-1 FILED. STOP — reviewer audits; ship #43 human-commit closes CP-ADOPT-3.**

### OPEN QUEUE: CP-ADOPT-3 CHECKPOINT-1 GREEN (reviewer) → ship #43 human-commit → defaults-insertion (author-trust slice, Arun ruling) → CP-ADOPT-4 (adopted authoring + palette opening + F-098 external-library oracle walk) → CP-ADOPT-5..7 → backend config audit (parallel, read-only) → Wave D/F/G → minimal ACT 2 → soak → tag 1.0.0 (early-to-mid December).

## ★ SHIP #43 SHIPPED 2026-09-17 = commit 75b3039 (parent 87be200) — 17 files — CP-ADOPT-3 (slots → drop zones)
Pillar C / H3 + H7. Slot descriptors (identity + child rules) emitted per zone; child-rules model {allowed,preferred,min,max,defaults,empty_display} from slot metadata + sidecar slot_rules overlay (H7: rules only, phantom id rejected+logged); adapter enforcement (Puck allow refusal + live min/max banner + empty_display via MosaicSlotZone); Columns column_1 ships min=1 (ruled dynamic exception). Filmed geometry album (banner/empty/non-overlap/before-after). Arun walk 3/3 PASS + WC#65 (below). Gates: Unit 2762/2762, Kernel 214/214, Vitest 552/1-B101, phpcs 0, phpstan L6, byte-identical 14e6cb9c, dist 1.0.31. SHIP-43.md written. **CP-ADOPT-3 CLOSED.**

## WALK-CATCH #65 (Arun, 2026-09-17) — sibling slot-zone appearance (behaviour correct)
When one sibling column holds the below-min banner and the other holds content, the two column zones are VERTICALLY MISALIGNED in the builder (left zone's content top-aligned, right centred), and the banner styling is rough. **Behaviour is correct** (the banner fires + clears correctly, filmed). Appearance only → **minimal ACT 2 ugliness list.** File refs: `js/src/builder/fields/MosaicSlotZone.tsx` (banner styling + zone body alignment) + the Columns canvas CSS (`.mosaic-columns__col` align-items). **Oracle add:** extend the cp-adopt-3 geometry oracle with a **sibling-zone y-alignment cell** (col1.y == col2.y within tolerance) so a future regression is caught. Tally **65**.

## DEFERRED RULING RECORDED — defaults[] auto-insertion (2026-09-17)
`defaults[]`-insertion-on-first-placement is deferred to the **author-trust slice**. **Reviewer lean (recorded, not yet ratified by Arun): insert the defaults on first placement, made VISIBLE as an unsaved change** (so the author sees + can undo it — honouring "never write silently"). Arun to confirm at the author-trust slice. Until then, `defaults` are emitted on the slot descriptor but not auto-inserted.

## CP-ADOPT-4 §P0 — HYBRID RENDER DERIVATION filed (2026-09-17) — REPORT-ONLY, no build pending reviewer audit
Fresh-read 75b3039 (ship #43 at HEAD, clean). Core 11.4.5. reports/REPORT-CP-ADOPT-4.md §P0 (Pillar D — render via core component element, hybrid; canvas assets; palette opens).
- **Render path:** MosaicRenderer:785 renders OWNED via direct Twig(getTemplatePath, {props,slots,meta,context,attributes,mosaic}) inside executeInRenderContext capturing #attached via BubbleableMetadata; attributes PRE-SEEDED (data-mosaic-*, visibility, spacing/style :752-773). Page auto-attaches core/components.<provider>--<id> per SDC.
- **LIVE PROBE:** (a) PAGE node/780 = 49 #attached libs (core/components.mosaic_components--<id> per instance + mosaic/renderer). (b) BUILDER route = mosaic/builder + media_library_bridge ONLY (NO component libs). (c) SSR JSON (CanvasPreviewController:113) = {"html":…} ONLY, renderSingleComponent(type,props):string — NO attachments. **THE GAP: the builder canvas has no path for a component's assets → adopted components render unstyled on the canvas.**
- **Core contract 11.4.5 (confirmed live):** #type=>component, #component, #props, #slots, #variant (SINGULAR → props.variant), **#attributes AVAILABLE (ComponentElement:170-184, merges element #attributes + props attributes)**, library auto-attach (core/components.<provider>--<id>), prop validation **assert-gated DEV-ONLY** (ComponentsTwigExtension:106) → Mosaic H5 stays the real gate.
- **Owned-template dependence = 19 templates** (12 mosaic_components + 7 other mosaic_* submodules; +1 doc stub). mosaic helper used by ZERO; attributes by all 19 (but renderer pre-seeds); slots var ONLY columns; nested props.* (MOSAIC-022); check_markup only mosaic_text (global). → owned stays direct-Twig (byte-identical), adopted → core element.
- **Hybrid switch / canvas assets / palette opening / DERIVED matrix (8 component × 6 surface × assets × geometry × parity × cache) / risk register (R1-R8: attachment ordering, autoescape, variant classes, #attributes avail, SSR size, prop-validation skew, attribute pre-seed loss, check_markup scope)** — all designed in the report. Byte-identical owned via region-shasum stays the gate.
- **P1 build order (5) recorded for the reviewer to gate.** **STOP — reviewer audits §P0 (hybrid boundary + canvas-asset gap + risks); NO build until then.**

### OPEN QUEUE: CP-ADOPT-4 §P0 reviewer audit → CP-ADOPT-4 P1 (hybrid switch + canvas assets + palette opening + adopted geometry/F-098 walk, byte-identical owned) → CP-ADOPT-5..7 → backend config audit (parallel, read-only) → Wave D/F/G → minimal ACT 2 (incl. WC#65 ugliness) → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-4 PASS 2 — P1a: HYBRID RENDERER (page) — CHECKPOINT-1 FILED (2026-09-18)
Fresh-read 75b3039 (ship #43 at HEAD, clean). Baseline 14e6cb9c. Ledger-first: risk register += R9 (core floor ^11.3 + Kernel cell), R10 (Drupal.attachBehaviors after SSR injection — P1b), R11 (builder attachment list carries the mosaic_component_library config cache tag — P1b) + teaser rule (raw/class-typed attributes prop NEVER in #props; all Mosaic data-attrs via #attributes only). **5 files (1 M + 4 new), PHP+YAML+Twig-fixture, NO dist/libs.**
- **Item 1 hybrid switch (DONE):** MosaicRenderer::renderNode branches at the render step. OWNED (provider mosaic/mosaic_*, or EMPTY→owned safe default) → direct Twig UNCHANGED. ADOPTED → buildAdoptedComponentElement → ['#type'=>component, '#component'=>$instance->type (NOT getTemplatePath — that DOUBLES the provider, bug caught+fixed), '#props' (H5 data minus attributes/variant/class-typed shapes = teaser rule/R7), '#slots' (rendered children as #markup Markup, keyed), '#variant' (enum→singular 11.4.5), '#attributes' (data-mosaic-* only; core merges, library wins R2/R3/R7)] inside the existing render-context harvest.
- **Item 2 attachments (DONE):** adopted render inside the harvest → core auto-attaches core/components.<provider>--<id> into the page BubbleableMetadata (R1 cell).
- **Item 3 fixtures (DONE):** Kernel HybridRenderTest 4/4 (36 assertions). NEW non-Mosaic fixture SDC `adopt_fixture:adopt_widget` (variant enum + required slot + class-typed raw prop) — **HONEST DEVIATION: charter said mosaic_test but mosaic_* is OWNED, can't reach the adopted branch → a non-Mosaic provider is required.** Adopted renders via core element: variant class, heading, owned-child in slot, data-mosaic-instance merged, raw prop DROPPED. olivero:teaser (real) renders its own theme markup + filled content slot + merged attr. R1 lib on page. Owned mosaic_heading still direct-Twig. RED: force isOwned=TRUE → adopted misroutes → LoaderError → restored.
- **Item 4 byte-identical (DONE):** node/780 14e6cb9c before==after; git diff --stat on the 19 owned templates = EMPTY (no owned template changed).
- **Item 5 gates:** Unit FULL 2762/2762 (fixed 8 mock regressions — unit renderer mocks carry no provider → empty→owned default restores them; NO test/oracle changed), Kernel FULL 218/218 (+4), phpcs 0. phpstan L6 MosaicRenderer = 14 errors ALL PRE-EXISTING (identical count in HEAD, missingType.iterableValue docblocks; my branch+method add ZERO — documented, not scope-crept on a 900-line hot-path file). No dist → libs 1.0.31.
- Report reports/REPORT-CP-ADOPT-4.md §P1a. **CHECKPOINT-1 FILED. STOP — P1b (canvas assets, SSR attachments, palette opening, adopted geometry + F-098 walk, dist) = next pass.**

### OPEN QUEUE: CP-ADOPT-4 P1a GREEN (reviewer) → P1b (canvas assets + SSR attachments + palette opening + adopted geometry/F-098 walk + dist) → CP-ADOPT-5..7 → backend config audit (parallel, read-only) → Wave D/F/G → minimal ACT 2 (WC#65 ugliness) → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-4 PASS 3 — P1b: PALETTE OPENS + CANVAS ASSETS + ADOPTED SSR — CHECKPOINT-2 FILED (2026-09-18)
Fresh-read 75b3039 (ship #43 at HEAD; P1a files uncommitted). Baseline 14e6cb9c. Built the SERVER-SIDE "adopted becomes usable" foundation + filmed the stable proof. **Full ADOPT-4 set now 14 files (9 M + 5 new incl. adopt_fixture), PHP+YAML+Twig-fixture, NO dist/libs.**
- **Item 3 palette opens (DONE):** adopt_palette = grade!=blocked (ComponentDefinition::fromCoreDefinition + discovery adopted-with-sidecar path). Governance reconciliation: adopted libraries default OFF (status=providerIsOwned); isAuthorable no-library adopted → FALSE (opt-in). LIVE: olivero library ON → admin palette=15, olivero:teaser (Attention) present; OFF/absent → excluded. Kernel PaletteOpenTest 4/4 (eligibility+Tier-B+descriptors; governance ON/OFF/no-library; restricted admin/author parity; SSR hybrid). RED: neuter no-library exclusion → fails → restored.
- **Item 2 canvas SSR (DONE server-side):** manifest sets adopted requires_ssr_preview=TRUE (routes to existing Tier-B, no dist). renderSingleComponent uses the hybrid (adopted → core element) — canvas SSR renders library markup not the double-provider error. PaletteOpenTest::testAdoptedSsrRendersViaHybrid.
- **Item 1 builder attachments (DONE):** MosaicLayoutWidget attaches core/components.<provider>--<id> for adopted palette components + config:mosaic.component_library_list cache tag (R11).
- **Item 4 FILMED (DONE — stable proof):** journey cp-adopt-4.spec.ts (scratch node 988, olivero:teaser + heading child). Album cp-adopt-4/ + INDEX: **j1 Component Libraries page (Olivero theme-bound ENABLED, teaser Attention 0 fields/5 slots), j3 anon page (teaser rendered w/ Olivero markup+CSS + Mosaic heading in content slot), page-computed.json (.teaser position:relative = olivero CSS applied, box 788x144), palette-ids.json (olivero:teaser in live palette).** VERIFIED j1+j3 visually. **HONEST DEFERRED (P1b-client): builder-canvas LIVE render didn't materialize headlessly (unstable, cp-adopt-1 precedent) — canvas SSR machine-proven at Kernel; client Tier-B SSR-attachments (R5/R10) + adopted-with-slots-on-canvas composition = next slice.**
- **Item 5 gates:** Unit FULL 2762/2762 (5 oracle-changes: adopted Ready/Attention palette-eligible), Kernel FULL 222/222 (+4), Vitest unchanged (552/1-B101, no JS), phpcs 0. phpstan L6: MosaicRenderer 14 + MosaicLayoutWidget 28 ALL PRE-EXISTING (identical in HEAD, ADOPT-4 adds ZERO). No dist → libs 1.0.31.
- Report REPORT-CP-ADOPT-4.md §P1b; WALK-CP-ADOPT-4.md (2 filmed steps: libraries page → page render, per WALK-SCRIPT LAW); SHIP-44-PLAN.md (14 files). **CHECKPOINT-2 FILED. STOP — reviewer audits; ship #44 human-commit closes the P1b server slice; P1b-client (canvas render+SSR attachments+dist) next.**

### OPEN QUEUE: CP-ADOPT-4 P1b server GREEN (reviewer) → ship #44 → P1b-client (canvas render + SSR attachments + attachBehaviors + dist) → adopted-with-slots-on-canvas composition → CP-ADOPT-5..7 → F-098 external-library oracle walk → backend config audit (parallel, read-only) → Wave D/F/G → minimal ACT 2 (WC#65) → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-4 PASS 4 — P1b-CLIENT: CANVAS RENDER OF ADOPTED (mechanism→fix→witness) — CHECKPOINT-3 FILED (2026-09-18)
Fresh-read 75b3039 (ship #43 at HEAD; P1a+P1b uncommitted). Baseline 14e6cb9c. Ship #44 was HELD until the canvas render was witnessed. **MECHANISM FIRST → FIX → RE-WITNESS.** Full ADOPT-4 set now **21 files (14 M + 7 new), dist rebuilt, libs 1.0.31→1.0.32.**
- **Item 1 WITNESS (no fix):** cp-adopt-4-witness.spec.ts opened the real builder (node 988). WITNESS.json BEFORE: **ssr:[] (no SSR request), .teaser count 0, generic dashed slot scaffold rendered; CSS WAS attached (teaser.css present).** **ROOT CAUSE (file:line): adapter's `slotKeys.length>0` generic-scaffold branch (MosaicPuckAdapter.ts:495) ran BEFORE the `requires_ssr_preview` Tier-B branch (:539) → adopted-with-slots never requested its SSR.**
- **Item 2/3 FIX:** reordered — requires_ssr_preview branch NOW precedes the generic scaffold; adopted-with-slots → buildAdoptedRenderer, no-slots → buildTierBRenderer, mosaic_view unchanged, owned unchanged. **Adopted slots composition:** renderSingleComponent emits `<mosaic-slot data-mosaic-slot=X>` markers per slot (server); NEW MosaicAdoptedPreview.tsx renders the library SSR chrome + PORTALS a Puck slot drop zone (MosaicSlotZone) into each marker → library markup + authorable slots; dropped child re-renders inside the library's own slot. Vitest MosaicAdoptedPreview.test.tsx 2/2; Kernel PaletteOpenTest asserts markers. **RE-WITNESSED GREEN: ssr:1(200), .teaser count 1, position:relative, mosaic-ssr-preview present, 0 errors.**
- **Item 4 FILMED:** album cp-adopt-4/ extended — **j2 canvas teaser (Tier-B SSR, content slot holds the Mosaic heading "Adopted Olivero"), canvas-computed.json position:relative box 288x440 = olivero CSS on the CANVAS matching the page.** VERIFIED j2 visually. All 5 journey cells pass.
- **Item 5 gates:** Unit FULL 2762/2762 (1 oracle-change Sprint66 Tier-A dispatch), Kernel FULL 222/222 (+SSR-marker cell), Vitest 554/1-B101 (+2 MosaicAdoptedPreview), phpcs 0, phpstan L6 MosaicRenderer 14 pre-existing (markers add ZERO), tsc only dsdShadow. **Byte-identical 14e6cb9c before==after.** Dist rebuilt (builder 1252kB + frontend-editor 782kB) → **BUMP-LIBS 1.0.31→1.0.32.**
- **DEFERRED (ledgered, refinement not blocker):** SSR {html,attachments} + client attach-once (dedupe R5) + Drupal.attachBehaviors (R10) for adopted components with DYNAMIC render-time attachments (static CSS already loads via route attach) → CP-ADOPT-5/6.
- Report REPORT-CP-ADOPT-4.md §P1b-client; WALK-CP-ADOPT-4.md rewritten (3 filmed steps: enable→canvas→page); SHIP-44-PLAN.md regenerated (21 files, dist 1.0.32). **CHECKPOINT-3 FILED. Ship #44 UNBLOCKED (canvas witnessed on screen). STOP — reviewer audits; ship #44 human-commit closes CP-ADOPT-4.**

### OPEN QUEUE: CP-ADOPT-4 GREEN (reviewer) → ship #44 human-commit → SSR-attachments refinement (R5/R10, dynamic-attach adopted) → CP-ADOPT-5..7 → F-098 external-library oracle walk → backend config audit (parallel, read-only) → Wave D/F/G → minimal ACT 2 (WC#65) → soak → tag 1.0.0 (early-to-mid December).

## CP-ADOPT-4R — WALK-CATCHES #66–#69 (Arun eye-test, 2026-09-18) — SHIP #44 BLOCKED
Arun walked the CP-ADOPT-4 P1b-client canvas render and caught four defects. My PASS-4 j2 frame showed the content child via FIXTURE STATE (node 988 layout had slots.content=[head-a4]) — NOT a proven drag. Tally **69**.
- **WC#66 (dirty on load):** the node form is marked dirty/unsaved immediately on opening an adopted component in the builder (no edit made) — something writes to Puck data during the adopted preview mount.
- **WC#67 (zone order / labels / banner):** the adopted component's slot zones render in the wrong order / without their labels, and the "requires at least 1" banner landed on the LAST zone instead of the zone whose descriptor carries the rule.
- **WC#68 (empty panel):** an adopted component with zero editable props (e.g. teaser — its only prop is the excluded `attributes`) shows an EMPTY property panel (just the unconditional sections), no guidance.
- **WC#69 (drop into adopted slot FAILS — BLOCKER):** dragging a real component into an adopted component's slot on the canvas does not work — the child never lands. The PASS-4 "it works" was fixture state injection, not a real drag.

## STANDING LAW — DROP-PROOF LAW (ratified 2026-09-18)
Any claim that a "drop zone" works / "drop works" / a child can be placed REQUIRES one of:
1. a **real-pointer drag** — `page.mouse.down/move/up` with `elementFromPoint` checks at the target, before/after DOM quoted; OR
2. the **keyboard-move path** (Puck's keyboard DnD), before/after DOM quoted.
**State injection (writing the layout JSON via drush/fixture, or `dispatch(setData)`) NEVER counts as drop proof.** An unfilmable/unprovable drop is marked **UNPROVEN** in the walk and **BLOCKS the ship**. Applies to every WALK-*.md from CP-ADOPT-4R forward. (Retroactive: the PASS-4 WALK-CP-ADOPT-4 slot-fill claim is downgraded to UNPROVEN pending WC#69.)

> **DROP-PROOF LAW addendum (2026-09-18, discovered in CP-ADOPT-4R):** Puck 0.21.3 registers **only `PointerSensor`** — there is **NO KeyboardSensor** (`@puckeditor/core/dist/chunk-YXFTA2VL.mjs:402,417-432`). So **option 2 (keyboard-move) is NOT available in this Puck version.** And the PointerSensor + new `@dnd-kit/react` 0.1.18 resolve zones via `document.elementsFromPoint` + a debounced collision observer that Playwright's synthetic pointer does **not** reliably drive — even the OWNED columns control never reaches `isEnabled` headlessly (`cp-adopt-4r/HARNESS-columns.json`, `CONTROL.json`). Net: a committed drop is currently **unfilmable in headless CI** for ANY zone; drop-proof must come from Arun's manual eye-test until/unless a working actuation is found.

## CP-ADOPT-4R — RESOLUTION (2026-09-18)
Fresh-read 75b3039. MOSAIC git READ-ONLY (edits only). Baseline `14e6cb9c…` (3954). JS-only source changes. Report `reports/REPORT-CP-ADOPT-4R.md`; album `e2e-evidence/cp-adopt-4r/` (+INDEX).
- **WC#69 (BLOCKER) — root-caused at SOURCE + proven cause FIXED; drop-COMMIT UNPROVEN.** Cause (file:line): Puck's `getPointerCollisions` contracts every dropzone box by `BUFFER=6`px/side before accepting the pointer (`chunk-YXFTA2VL.mjs:3233-3244`) → a **zero-width** zone (`contractedBox.left>contractedBox.right`) can **never** be a drop candidate → never `isEnabled` → droppable `disabled` → structurally undroppable. Empty adopted slots rendered ZERO width because the library's layout CSS is absent on the canvas + `MosaicSlotZone` enforced no width. FILMED: `PRECISE.json` image `width:0` → `RECTS.json` all 5 slots `passesBuffer:true` (image `0→96`). FIX: `MosaicSlotZone.tsx` guarantees a 96×48 outlined hittable box while empty. **Honest limit:** the committed drop cannot be filmed headlessly (no keyboard sensor; synthetic pointer doesn't drive the new dnd-kit — owned columns control ALSO never enables a zone, `HARNESS-columns.json`/`CONTROL.json`). Per DROP-PROOF LAW → **UNPROVEN → ship #44 BLOCKED** pending Arun's manual re-test.
- **WC#67 — FIXED + filmed.** Inline slots (no portals, `htmlToReactSlots.tsx`) render in twig order; each `MosaicSlotZone` binds its slot id + shows its label + gets its OWN descriptor (so the min banner lands on the ruled zone). `WC67-LABELS.json`: prefix/meta/image/title content/content in order.
- **WC#66 — NOT reproduced (open).** `WC66-68.json`: textarea stable on pristine load (`changedOnLoad:false`); `serializeForDirty` already strips Tier-B preview keys. No witnessed mechanism → no fix shipped (MECHANISM-FIRST). Data smell ledgered: teaser `title` SLOT stored as `props.title` array. Needs Arun repro.
- **WC#68 — witnessed, fix scoped, NOT built (open).** `WC66-68.json`: props-less teaser panel shows only 10 meta controls, no component field. Fix = "built from its slots" empty-state gated on no-authorable-field. Next pass.
- **Composition hardening:** `MosaicAdoptedPreview.tsx` now inline (no `createPortal`) + memoised tree (stable DOM identity). Diagnostic experiments (pink scaffold, resolveData-off) applied then REVERTED.
- **Gates:** Vitest 554/1-B101 (field specs 27/27); phpcs 0-err (2 pre-existing warns); phpstan L6 39 (all pre-existing dependencySerialization systemic pattern, JS-pass adds ZERO); Adopt Kernel 24/24; **full Kernel+Unit 2984/0** (1 warn, 6 deprec, non-fatal); **byte-identical `14e6cb9c` before==after**. Dist rebuilt → **BUMP-LIBS 1.0.32→1.0.39**.
- **STOP — ship #44 stays BLOCKED (WC#69 drop UNPROVEN). WC#67 fixed; WC#66/#68 open. Arun manual re-test of the min-box fix is the gating next step.**

## CP-ADOPT-4R PASS 2 — WC#66/#68/#69/#70 RESOLVED, WC#69 DROP PROVEN (CDP) — SHIP #44 UNBLOCKED (2026-09-19)
Fresh-read 75b3039. MOSAIC git READ-ONLY (edits only). Baseline `14e6cb9c` (3954). Report `reports/REPORT-CP-ADOPT-4R-P2.md`; walk rewritten `reports/WALK-CP-ADOPT-4.md` (story, Arun creates the page, drops PROVEN via CDP); `reports/SHIP-44-PLAN.md` regenerated; album `e2e-evidence/cp-adopt-4r/`.
- **WC#69 (BLOCKER) — FIXED + PROVEN with real CDP input.** PASS-1 min-box was necessary but INSUFFICIENT (Arun: drop still failed). Built a working real-input harness (CDP `Input.dispatchMouseEvent`; owned columns control LANDS — `CDP-columns.json`; tall viewport so palette+zone coexist; PointerSensor 200ms-hold activation; live mid-drag zone re-measure). THREE stacked causes, each proven with instrumented Puck: (1) zero-width slots vs `getPointerCollisions` BUFFER=6 (PASS-1 min-box); (2) library CSS (`teaser__image position:absolute`, flex/grid) OVERLAPS zones + pushes them outside the component box → `findDeepestCandidate` never sees the inner dropzone (resolves to component→root:default-zone) — `EFP.json`/`MDBG-*` — FIX: `.mosaic-adopted-preview` canvas layout reset (block flow), `GEO.json` shows clean stacked zones; (3) 5 stacked empty slots each reserve `--min-empty-height:128px` → ~640px reflow during drag shoves the target off-pointer (image→enabled `meta`) — FIX: cap `--min-empty-height`+min-box to 32px in `.mosaic-adopted-preview`. PROOF `WC69-CDP-PROOF.json`: image empty→`slots.image=[mosaic_divider-…]`; content→`slots.content=[mosaic_heading-…,head-a4]`; before/after DOM quoted; correct slot; columns control still lands.
- **WC#66 — REPRODUCED + FIXED.** `WC66-adopted.json` pristine /node/988/edit → View → beforeunload `dialogFired:true` (owned 987 false). Mechanism: Puck seeds empty SDC slots as `props[slot]=[]` on mount (`"props":{"title":[],prefix:[],meta:[],image:[]}`), absent from the server baseline → serializeForDirty mismatch. FIX: `serializeForDirty` strips empty-array props (dirty comparison ONLY; SAVE path keeps F-089 untouched). After: `dialogFired:false`.
- **WC#67 — FIXED (PASS 1, confirmed).** `WC67-LABELS.json`.
- **WC#68 — BUILT.** Props-less adopted panel shows "No fields — this component is built from its slots" + slot list w/ required marker (`MosaicSlotInfo.tsx`). `WC68.json` + `wc68-empty-panel.png`. Vitest 2/2.
- **WC#70 — VALIDATOR + RENDERER (rulings a+b).** (a) `MosaicPropValidator::validateNode` (via validateFull → entity constraint + controllers + presave hook) refuses an empty SDC-`required` slot, message names component+slot. NO owned component declares a required slot (verified src+all submodules) → zero owned-fixture impact; Columns `column_1 min=1` is a child-rule not SDC-required, left to its client banner (no fixture broken). (b) `MosaicRenderer::renderNode` skips an instance with an empty required slot (renders nothing + logs) → front end never shows broken markup (Arun node 989). Kernel `RequiredSlotTest` 4/4; Adopt Kernel 28/28.
- **Gates FULL:** full Kernel+Unit **2988/0** (8065 assert; 1 warn/6 deprec non-fatal); Vitest 556/1-B101 (+2 MosaicSlotInfo); phpcs 0-err on changed files; phpstan +0 (MosaicRenderer 3 pre-existing); **byte-identical `14e6cb9c` before==after**. Dist builder+frontend-editor rebuilt → **BUMP-LIBS 1.0.39→1.0.42**.
- **STOP — ship #44 UNBLOCKED. WC#66/67/68/69/70 all resolved; WC#69 drop PROVEN via CDP. Arun's walk (WALK-CP-ADOPT-4.md) is the gating next step → human commit.**

## STANDING LAW — PROOF-CONDITIONS LAW (ratified 2026-09-19, Arun's 4th WC#69 report)
A drop proof counts ONLY when reproduced in the WALK's real conditions, ALL of:
1. a **fresh page created in-session** (`/node/add/page` → place the component here — NOT a pre-seeded/fixture node whose ids you hand-crafted);
2. **served bundle == built dist** — quote `js.preprocess`, the libs `?v=` served vs `mosaic.libraries.yml`, and sha256(served)==sha256(built);
3. a **real browser session** — Playwright **HEADED Chrome** (`channel:'chrome'`, `headless:false`), NOT headless Chromium;
4. **Arun's exact step order**.
A proof that passes only on a fixture (hand-crafted ids), or only headless, or on a stale bundle, is a **test-coupling failure** — mark UNPROVEN, do not claim proven. (Retroactive: the PASS-2 CDP "PROVEN" mark is WITHDRAWN — it passed on the colon-free fixture id `teaser-a4`, hiding the colon bug.)

## CP-ADOPT-4R PASS 3 — WC#69 TRUE CAUSE (colon in placed id) FIXED + HEADED-PROVEN (2026-09-19)
Fresh-read 75b3039. MOSAIC READ-ONLY. Baseline `14e6cb9c`. JS-only. Report `reports/REPORT-CP-ADOPT-4R-P3.md`.
- **STEP 0 stale check:** served==built. `js.preprocess=false`; sha256(served `?v=1.0.42`)==sha256(built)==`040e9cc0…784e5e`. NOT stale — Arun's 1.0.42 had the PASS-2 code (necessary, insufficient).
- **CAUSE (file:line):** a PLACED adopted teaser's instance id is `olivero:teaser-<uuid>` (CONTAINS A COLON). Puck `generateId=(type)=>\`${type}-${uuid}\`` (`@puckeditor/core/dist/chunk-6XCJ3Y7O.mjs:46`) + zone-id parse by `split(":")` (`chunk-YXFTA2VL.mjs:5786`, :3737-3738,:4709,:6125) → `olivero:teaser-<uuid>:content` splits at the WRONG colon → slot never resolves → `dragOver`→component→root:default-zone, drop never lands. The node-988 FIXTURE used a hand-crafted colon-free `teaser-a4` which HID it (the test-coupling failure the PROOF-CONDITIONS LAW targets).
- **FIX (`MosaicPuckAdapter.ts`):** register adopted under a colon-free Puck key; `:`⇆`--` (SDC ids have no hyphens → unambiguous; owned identity-mapped). `toPuckType`/`fromPuckType` + 4 points: `components[toPuckType(id)]`, category push, `nodeToPuckItem` encode, `fromPuck` decode. Internal `id` stays real for owned-branch checks + SSR `component_id` (renderSingleComponent unaffected). Saved layout keeps real `olivero:teaser`.
- **HEADED PROOF (PROOF-CONDITIONS LAW):** headed Chrome, `/node/add/page`→title→Start-blank→place Teaser→drag Heading into Content. `HEADED-PROOF.json` 3/3 `landed:true before:0→after:1 slots.content=["mosaic_heading-…"]`, placed id colon-free `olivero--teaser-<uuid>`; `DRAG-SAMPLE.json` instrumented: content dropzone now a candidate (dt:true), `dragOver targetZone=…:content`, `dragEnd previewKeys=[…:content]`. `CLEAN landed=true` on the shipped un-instrumented build. **served==built for 1.0.43** (`d7429dc5…95e7875a`). Reflow (32→68px on hover) tolerated. `FRESH-GEO.json`: at rest all zones colon-free, non-overlapping, reachAtCenter:true.
- **Gates:** Vitest 561/1-B101 (+5 MosaicPuckAdapterColonType); **byte-identical `14e6cb9c` before==after**; full Kernel+Unit unchanged (JS-only, no PHP); dist rebuilt clean → **BUMP-LIBS 1.0.42→1.0.43**.
- **STOP — ship #44 unblocked; WC#69 HEADED-PROVEN under the PROOF-CONDITIONS LAW. Arun's own walk (WALK-CP-ADOPT-4.md) is the gate → human commit.**

## SHIP #44 SHIPPED — CP-ADOPT-4 + 4R (0bcc2ab) — 2026-09-19
Commit **0bcc2ab** (parent 75b3039 = ship #43), **29 files**, tracked tree clean == HEAD. Walk **7/7 PASS** (Arun). byte-identical `14e6cb9c` held; libs **1.0.43** (served==built `d7429dc5…`). Full ledger `ledger-live/SHIP-44.md`. **WC#66–#70 CLOSED** (dirty-on-load, labels, empty-panel, drop-into-slot HEADED-PROVEN + colon-free Puck key, empty-required-slot validator+renderer). Gates: Kernel+Unit 2988/0, Vitest 561/1-B101. **Tally 72.** THE ARC: adopt any SDC authorable end-to-end (Olivero teaser placed, slots filled by real drag, empty required slot refused, saved page matches, owned byte-identical).

### WC#71 (Arun, save-rejection wording) — author-trust slice, OPEN
A save-rejection surfaced by the H5 validator / entity constraint exposes INTERNAL detail (node ids, raw "the JSON is invalid") to the author. Ruling: author-GRADE wording — name the component + what to do ("Add content to the Teaser's Content area"), never an id or "JSON". Scope: author-trust slice (small, pre-tag). Touches MosaicPropValidator messages + the widget's setError surface + the constraint validator. Why: an author must never see the machine's guts. How to apply: a message-mapping layer that turns `Node <uuid>: the 'olivero:teaser' component requires content in its 'content' slot.` → `The Teaser needs content in its Content area.` (human label from the manifest, no uuid).

### WC#72 (Arun, clipped field on canvas) — geometry, ACT 2 (reviewer to size)
A field renders HALF-VISIBLE (clipped) inside the adopted preview on the canvas. Geometry defect. Ruling: ACT 2 UNLESS it HIDES content the author needs (then pre-tag). Reviewer sizes on Arun's detail (which field, which library, does it hide or just look ugly). Candidate cause: SO-8 overflow discipline — the canvas chrome or `.mosaic-adopted-preview` clips a library field; or the library's own `overflow:hidden` cropping under the reset. Fold into ADOPT §9 SO-8 acceptance.

### ACT-2 consistency item — adopted zones show slot labels, owned Columns zones don't
`MosaicSlotZone` shows a slot LABEL for adopted (WC#67); owned Columns column_1/column_2 zones render without a label. Inconsistent affordance. ACT-2: either add labels to owned container zones too, or gate labels behind an "adopted / multi-slot" heuristic. Cosmetic, not blocking.

### RC-A3 (keyboard-drag spike) — NEW INPUT: Puck 0.21 has NO KeyboardSensor
ADOPT §8 rider RC-A3 (Pillar C child rules MUST be keyboard-operable; builder keyboard-drag = the RC-A3 spike, Wave F). **NEW CONSTRAINT (CP-ADOPT-4R):** Puck 0.21.3 registers ONLY `PointerSensor` — there is NO KeyboardSensor (`@puckeditor/core/dist/chunk-YXFTA2VL.mjs:402,417-432`). So keyboard-drag is NOT available out of the box; the RC-A3 spike must either add a KeyboardSensor to Puck's DragDropProvider, provide a keyboard slot-fill path OUTSIDE dnd-kit (panel-driven add-to-slot), or accept a documented a11y gap. Sizes the spike input. Wave F.

### PACKET — ADOPT §9 Style Ownership + Cascade Discipline (DRAFT, delegated ruling O4, overrule open)
Appended `reports/ADOPT-DESIGN.md §9`: walls (Shopify specificity study, Canvas layer/preflight collisions, Builder noWrap, Gutenberg contentOnly, Shadow-DOM slot ownership); contracts **SO-1..SO-8**; scenarios **S1–S18**; options **O1–O4 ruling O4** (`@layer` cascade discipline + one scoped low-specificity author layer + edit-mode-only canvas reset + overflow discipline + optional per-library Shadow-DOM escape hatch SO-7); movie; §5 acceptance additions. Scope: **CP-ADOPT-5** (SO-1..6,8) **+ SO-7 in CP-ADOPT-6**. Consistent with H1 (saved type stays `provider:id`; colon-free key is Puck-internal only) + H2 (decorate, never fork). **Bible P2 defensible-seat line += "style ownership with cascade discipline (ADOPT §9)".**

### OPEN QUEUE after #44: WC#71 (author-trust) → WC#72 sizing → CP-ADOPT-5 (Style Ownership SO-1..6,8) → CP-ADOPT-6 (SO-7 + SSR-attach R5/R10) → backend config audit (parallel, read-only) → Wave D/F/G (RC-A3 spike) → minimal ACT 2 (WC#65, WC#72, label consistency) → soak → tag 1.0.0.

## CP-ADOPT-5 P0 — ledger (2026-09-19)
- **ADOPT §9 Style Ownership: DRAFT → RATIFIED 2026-09-19** (Arun: "I agree with your solution, it's your call"). Ruling O4 stands; SO-1..SO-8 pinned. Bible P2 defensible-seat line stands (already carries "style ownership with cascade discipline (ADOPT §9)").
- CP-ADOPT-5 P0 = DERIVATION only (report-only, MOSAIC read-only, no dev DB/config writes). Report `reports/REPORT-CP-ADOPT-5.md`: Pillar E typed binding + capability-aware panel + §9 SO-1..8 + H9 slot binding. STOP for audit before any build.
- **CP-ADOPT-5 P0 report FILED** `reports/REPORT-CP-ADOPT-5.md` (derivation, report-only). Capability table = proposed truth (10 kinds × 4 caps; today's panel is kind-AGNOSTIC/dishonest — binds enums, breakpoint-overrides media as text). Binding = 7 DataSourceBinding types + 6 Views arg sources (distinct). **H9 slot-into-View UNBUILT** (`views_row_sdc` 0 code hits). **@layer CORRECTION: NOT absent — already in production, 11 css files** (FE order `mosaic-design-system.css:23` tokens<components<site-theme; canvas `mosaic-canvas-reset.css:30` admin<…); design RECONCILES the existing order, does not introduce it; author overrides already win via inline custom props. 10 risks; R10 (binding-honesty behaviour change → migrate saved layouts) + R2 (layer computed-style oracle) flagged for Arun. Build order P1a(caps+gating)→P1b(bare+plain+layers)→P1c(H9)→P1d(walk). STOP for audit.

## CP-ADOPT-5 PASS 2 — P1a rulings (2026-09-19)
- **R10 RULING (binding-honesty behaviour change):** GRANDFATHER. A stored binding on a now-non-bindable kind (select/toggle/repeatable/raw) is STILL HONOURED by the renderer (never silently dropped); the panel shows "Legacy binding — remove to edit" instead of the editable binding row; a `legacy_bindings` list is emitted per layout for the Pillar G report. No migration that erases author intent.
- **R2 RULING (layer computed-style drift):** a COMMITTED `scripts/qa/style-shasum.sh` — fixed selectors × fixed computed properties on node/780 — is the SECOND gate beside region-shasum. Region shasum (HTML) + style shasum (computed CSS) must BOTH be identical across a pass that intends no visual change.
- **§9 note:** @layer is already present (11 css files, order declared design-system.css:23 / canvas-reset.css:30); SO-4/SO-5 RECONCILE the existing order (P1b), they do not introduce it. P1a = panel + capability metadata ONLY, no CSS change → both shasums identical after P1a.

## CP-ADOPT-5 P1a — CHECKPOINT-1 FILED (2026-09-19)
Report `reports/REPORT-CP-ADOPT-5.md §P1a`. MOSAIC git READ-ONLY (edits only; Arun commits). Capability rules + capability-aware panel gating + grandfather.
- **STEP 0 oracles:** committed `scripts/qa/style-shasum.sh`(+`.mjs`) = R2 second gate. Baselines: REGION `14e6cb9c…3954`, STYLE `b7756795…ca982 4354 10`. **BOTH IDENTICAL after P1a** (panel+metadata only, no render/visual change).
- **PHP:** `PropShape::capabilities(kind)` (P0 table) + `PropDescriptor::toArray` emits `capabilities{bind,breakpoint,style,translate}` per prop_descriptor; `MosaicCapabilityAudit::legacyBindings` (grandfather detector). Unit PropShapeCapabilityTest 12/12, Kernel CapabilityAuditTest 3/3.
- **ADAPTER:** adopted shows Data only if bindable-kind prop, Breakpoint only if breakpointable; OWNED keep EXACTLY today's 5 sections (**field-set diff EMPTY**). `foreignSlotResolveFields` hides spacing/style/responsive + "Styling is owned by {Library} {Component}" when parent adopted, unchanged otherwise. Legacy binding → "Legacy binding — remove to edit". Vitest MosaicPuckAdapterCapability 6/6 + MosaicDataSourceFieldLegacy 2/2.
- **GRANDFATHER (R10):** renderer honours a legacy binding (Kernel: select `level` → `<h2>`); audit emits the list; panel shows read-only "remove to edit". Add-dropdown honesty deferred to P1b (no owned add-flow touched).
- **FE parity:** gating computed in toConfig from the shared manifest capabilities → admin==FE by construction.
- **Gates:** Kernel+Unit **3003/0**, Vitest 569/1-B101 (+8), phpcs 0-err + phpstan 0-err on changed files, **both shasums identical**, dist rebuilt → **BUMP-LIBS 1.0.44**.
- **STOP — CHECKPOINT-1 filed. P1b (bare render, plain content, @layer reconcile / scopes / tokens) next.**

## CP-ADOPT-5 P1b — CHECKPOINT-2 FILED (SO-1 delivered; cascade gate-analyzed) — 2026-09-19
Report `reports/REPORT-CP-ADOPT-5.md §P1b`. MOSAIC READ-ONLY. **PARTIAL P1b (honest).**
- **SO-1 BARE RENDER — DELIVERED + Kernel-proven, both gates held.** `MosaicRenderer::renderNode($bare)`: a child inside an adopted slot renders with NO Mosaic wrapper/spacing/style/visibility, only `data-mosaic-instance`; CID keyed `:bare`; owned children unchanged. BareRenderTest 2/2. **REGION `14e6cb9c…3954` + STYLE `b7756795…ca982 4354 10` IDENTICAL** (node/780 owned-only, untouched). Markup: inside teaser `<h3 class="mosaic-heading…" data-mosaic-instance="h-1">` (no data-mosaic-component, no --mosaic-pt); top-level keeps both.
- **Font-family MEASURED:** `.mosaic-heading{font-family:var(--mosaic-font-heading)}` (`mosaic-design-system.css:199`, @layer mosaic-components) is GLOBAL → even bare inside the teaser the Heading still = `system-ui,-apple-system,'Segoe UI',Roboto,sans-serif` (Mosaic's), NOT Olivero's. Olivero inheritance needs **SO-5 @scope** (`@scope … to ([data-mosaic-component*=":"])`) — NOT yet landed.
